...
The protocol treats `message` as an opaque Claude-facing string.

Prompt-composition rules such as "send this block verbatim" are Codex skill behavior: Codex chooses the exact `message` value before calling `tell_claude`, `ask_claude`, `tell-file`, or `ask-file`.
...
An explicit target may be an endpoint id, unique display name, project path, or numeric index from the current list output.

Endpoint ids are the durable protocol-facing selector.
...
The raw local HTTP bridge does not require a target field in the body because target resolution happens in the CLI or Codex MCP adapter before the request is sent.

Adapter responses include `target` so callers can audit which Claude Code endpoint was used.
...
Metadata values must be non-empty strings of at most 200 characters. Control characters and these attribute-breaking characters are rejected:

```text
< > " ' `
```

The HTTP bridge validates the user-influenced `sender` value at ingress. Unsafe sender values fall back to `codex`.

System metadata such as `request_id`, `reply_required`, and `received_at` is generated by the channel server and must remain valid under those rules.
